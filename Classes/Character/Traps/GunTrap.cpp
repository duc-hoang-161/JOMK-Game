...
{
	m_sprite = NULL;
	m_bullets = new Bullets(5, "Characters/Bullet/Bullet04/BulletAnimation.json");
...
void GunTrap::collision(LivingObject *lObj, void *data)
{
	if (canAttack(lObj, 0)) {
...
void GunTrap::setAttribute(const rapidjson::GenericValue < rapidjson::UTF8<char>, rapidjson::MemoryPoolAllocator
	< rapidjson::CrtAllocator >> *attribute)
{
	if (attribute->FindMember("Sprite") != attribute->MemberEnd()) {
		...
		if (sprite->FindMember("TimeShoot") != sprite->MemberEnd())
			time=((*sprite)["TimeShoot"].GetFloat());
	}
...
void GunTrap::shot()
{
	if (m_canShot) {
		...
		cocos2d::Vec2 direct;
		direct.set(0, 0);
		currentPos.x -= m_sprite->getContentSize().width;
		direct.x = -600;
		...
		m_canShot = false;
		Helper::waitAndExecute(time, [&]() {
			m_canShot = true;
		});
	}

}
void GunTrap::addParent(Layer *layer)
{
	Trap::addParent(layer);
	m_bullets->addParent(layer);
}
void GunTrap::createPhysicsBody(b2World *physicsWorld)
{
	Trap::createPhysicsBody(physicsWorld);
	m_bullets->createPhysicsBody(physicsWorld);
}
void GunTrap::update(float datatime)
{
